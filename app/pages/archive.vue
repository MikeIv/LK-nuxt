<script setup lang="ts">
  ...
  onMounted(async () => {
    /*const data = await recordStore.getReportsList()
  console.log(data)*/
  ...
  const columns: TableColumn<Record>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return `${new Date(row.original?.period?.start).toLocaleString(
          "ru-RU",
          {
            day: "numeric",
            month: "numeric",
          },
        )} - ${new Date(row.original?.period?.end).toLocaleString("ru-RU", {
          day: "numeric",
          month: "numeric",
          year: "numeric",
        })}`;
...
`;
  ...
  function getHeader(column: Column<Record>, label: string, columnClass) {
